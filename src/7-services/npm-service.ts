...
function installPackageAndPack(
  packageName: string,
  ...
  if (options?.recursive === false) {
    ...
    for (const dep of packageDependencies) {
      installPackageAndPack(dep, options);
    }
  }
...
function packageToZip(
  ...
  installPackageAndPack(mainPackage, options);
  logService.logDivider(logService.packageData.logFilePath);
  logService.logMsg(
    logService.packageData.logFilePath,
  ...
  logService.logMsg(
    logService.packageData.logFilePath,
    `not packed: ${logService.packageData.notPackedPackages.size} packages: [${[
      ...logService.packageData.notPackedPackages,
    ].sort()}]`
  );
  logService.logMsg(logService.packageData.logFilePath, `zipping all packages`);
  logService.logDivider(logService.packageData.logFilePath);
